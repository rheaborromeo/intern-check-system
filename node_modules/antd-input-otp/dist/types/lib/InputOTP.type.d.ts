...
type TInputType<T extends 'custom' | 'standard'> = T extends 'custom' ? 'custom' : 'numeric' | 'alphabet' | 'symbol' | 'alphabet-symbol' | 'alphabet-numeric' | 'numeric-symbol' | 'all';
type TInputRegex<T extends 'custom' | 'standard'> = T extends 'custom' ? string | RegExp : never;
export type TAutoSubmit<T extends 'uncontrolled' | 'controlled'> = T extends 'uncontrolled' ? FormInstance : (value: string[]) => void;
interface BaseInputOTPProps extends Omit<InputProps, 'className' | 'form' | 'onChange' | 'style' | 'value'> {
    /**
...
export type TGetSibling = Record<'nextTarget' | 'prevTarget', (EventTarget & HTMLInputElement) | null>;
type TIsNotTheCharacterProps = {
    inputType: NonNullable<InputOTPProps['inputType']>;
    inputRegex: InputOTPProps['inputRegex'];
    value: string;
};
export type TIsNotTheCharacter = ({ inputType, inputRegex, value, }: TIsNotTheCharacterProps) => boolean;
export {};
